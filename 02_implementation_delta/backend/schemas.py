...
class DetachInfo(BaseModel):
    """分手戒断状态：理性已确认不复复合、情感尚未戒断的阶段与触发强度。"""
    stage: str = "应激期"  # 应激期/戒断行动期/稳定坚持期/复发应对期/重建转化期
    weeks_since_breakup: int = 2  # 分手至今周数
    relationship_months: int = 24  # 关系时长（月）
    contact_urge: int = Field(default=7, ge=1, le=10)  # 近两周为止的联系冲动强度
    relapse_count: int = 0  # 累计复发/破戒次数
    safety_flag: bool = False  # 是否已触发高危信号（自伤/他伤/失控）
...
class Persona(BaseModel):
    ...
    family: str = ""
    income: str = ""
    detach: DetachInfo = Field(default_factory=DetachInfo)
