"""LLM 逐人生成全新人格：16 原型仅作 JTBD 结构槽，带 QC 与修复重试。"""
...
def _enforce_blueprint_structure(persona: Persona, blueprint, factors: List[Factor]) -> Persona:
    """通过 QC 后仍强制对齐 Job/分群/角色骨架，杜绝「分群→Job」类漂移入库。"""
    ...
    if bp_outcomes:
        ...
        if len(kept) < 2:
            ...
            persona.jtbd.desired_outcomes = [
                d for d in persona.jtbd.desired_outcomes if d.id in allowed
            ]
