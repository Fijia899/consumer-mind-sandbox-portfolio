...
def _voice_card(persona: Persona) -> str:
    ...
    det = persona.detach
    det_brief = (
        f"{det.stage or '应激期'}·分手{det.weeks_since_breakup}周·"
        f"联系冲动{det.contact_urge}/10·复发{det.relapse_count}次" 
        + ("·高危" if det.safety_flag else "")
    )
...
【硬性写作禁令 — 违反则不合格】
1. **禁止**所有人共用的套话开头，例如：「这个方案挺好的？」「你们说得对？」「正好戳中我…」「这段话提到了…」
...
5. 不同人关注点必须不同：夜间脆弱谈深夜复发/睡不好；反复联系谈「消息要发出去」的冲动；等待复合谈不甘心与侥幸；孤独依赖谈没人接住/害怕打扰；自我价值受损谈不配被爱；高危型谈怕失控/怕被当矫情——不要人人都只谈「怕戒不掉」。
...
【口吻示例（仅示意差异，勿照抄）】
- 反复联系型：「昨晚又编辑好一条消息，5分钟没发出去。你们说的延迟缓冲，能帮我熬过那五分钟吗？」
- 夜间脆弱型：「凌晨三点又醒了，满脑子都是TA。睡前仪式真的有用？不是又给我上鸡汤吧。」
- 孤独依赖型：「周末一个人窝着，想找人说说话又怕打扰朋友。有人懂，是真的有人听我说完不评判吗？」
- 高危型：「我有几次闪过伤害自己的念头，不敢跟任何人说。你们说24小时热线，打过去真的有人接吗？」
