...
def _maybe_advance_stage(persona: Persona, result: SimulateResult) -> None:
    """根据多次经历推动戒断阶段（Day 1+ 人会变）。"""
    stage = persona.detach.stage
    ...
    hesitates = sum(1 for m in recent if m.decision == "hesitate")

    new_stage = stage
    if result.decision == "advance" and result.willingness and result.willingness >= 7:
        if stage == "应激期" and advances >= 1:
            new_stage = "戒断行动期"
        elif stage == "戒断行动期" and advances >= 2:
            new_stage = "稳定坚持期"
        elif stage == "复发应对期" and advances >= 2:
            new_stage = "重建转化期"
    elif result.decision == "hesitate" and stage == "稳定坚持期":
        if hesitates >= 2:
            new_stage = "复发应对期"

    if new_stage != stage and STAGE_ORDER.index(new_stage) > idx:
        persona.detach.stage = new_stage
...
def _maybe_reflect(evo: PersonaEvolution) -> None:
    ...
    if adv >= 2:
        belief = "综合来看，我最近对「彻底放下」越来越有信心，可能在近期做出关键改变。"
    elif hes >= 2:
        belief = "综合来看，我仍在徘徊——安慰听了不少，但核心心结还没被真正解开。"
    elif decisions.count("reject"):
        belief = "综合来看，我对这类鼓动越来越警惕，短期不太会照着做。"
    else:
        belief = "综合来看，这件事还在我生活的边缘，没有强烈的行动意愿。"
...
DEFAULT_STAGE_AFTER_RESET = "应激期"


def reset_persona_journey(persona: Persona) -> None:
    """清除记忆后，将戒断阶段恢复为 Day 0 基准。"""
    persona.detach.stage = DEFAULT_STAGE_AFTER_RESET
    persona.detach.relapse_count = 0
